...
struct InnerStockPosition {
    ...
    std::string code_;
    int64_t total_borrowed_volume_ = 0;  // 今日融券卖出的总额度, 不会变化
    ...
    int64_t total_sell_volume_ = 0;      // 今日普通卖出的总额度, 不会变化
...
class InnerStockMaster {
 ...
 protected:
    ...
    InnerStockPositionPtr GetOrCreatePosition(std::string code);

 private:
    std::map<std::string, InnerStockPositionPtr> positions_;
