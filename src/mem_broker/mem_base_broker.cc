    ...
    void MemBroker::SendTradeOrder(MemTradeOrderMessage* req) {
        ...
        try {
            ...
            if (trade_type == kTradeTypeOption && is_batch) {  // 期权暂不支持批量报单，因为自动开平仓逻辑需要支持才行
                throw std::runtime_error("[FAN-BROKER-ERROR] option batch order is not supported yet!");
            }
            if (items_size > account->batch_order_size) {
                throw std::runtime_error("[FAN-BROKER-ERROR] too many orders in request: " + std::to_string(account->batch_order_size) + ", limit is 1000");
            }
            ...
            for (int i = 0; i < items_size; ++i) {
                ...
                if (strlen(order->code) == 0) {
                    throw std::runtime_error("[FAN-BROKER-ERROR] code is required");
                }
                int64_t market = co::CodeToMarket(order->code);
                if (market <= 0) {
                    throw std::runtime_error("unknown market suffix of code: " +  string(order->code));
                }
    ...
    void MemBroker::SendTradeWithdraw(MemTradeWithdrawMessage* req) {
        try {
            if (strlen(req->order_no) == 0 && strlen(req->batch_no) == 0) {
                throw std::runtime_error("[FAN-BROKER-ERROR] order_no or batch_no is required");
            }
            OnTradeWithdraw(req);
        } catch (std::exception & e) {
            std::string error = e.what();
